...
/**
 * VIP se distingue de Premium (violeta/dorado sutil, no cyan) — debe sentirse exclusivo, no un
 * warning genérico. Gratis se mantiene neutral/discreto. Ver punto 22-23 del rediseño.
 */
export default function PremiumBadge({ access, isPremium, className = "" }: PremiumBadgeProps) {
  const isVip = access.toLowerCase().includes("vip");
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold backdrop-blur-md ${styles} ${className}`}
    >
      {isPremium && (
